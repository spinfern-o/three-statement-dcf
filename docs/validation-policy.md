...
| `VAL-017-012` | 17.12 | `SCH-INTANGIBLES` ending balance = intangibles on the balance sheet (13.3) | ERROR | **Yes** — closing F-17 gave the chart `intangibles` and a separate `amortization` line, so the roll-forward exists and this reconciles like 17.11 and 17.13. A filing reporting no intangibles skips for that reason, which is a fact about the filing |
...
| `VAL-017-029` | 17.29 | Every displayed rounded value ties to its full-precision stored value (4.18, 4.19) | ERROR | **Yes.** The check parses the displayed string back and asserts it equals the stored decimal at the display precision, that a rounded value carries 4.19's full-value tooltip, and that a lossless one does not claim one. It previously compared a pure function with itself and could only pass (finding F-38). 4.19's tooltip still has no CLI analogue |
