...
| 11.2 | Preserve every original company label | **Yes** — `ReportedFact.raw_label` is never overwritten, and the mapping sits beside it. The label is re-read from the page rather than stitched from table cells, because a wide title can split it across two columns and neither joining nor not joining the fragments is right for both a split on a space and one mid-word (finding F-18) | **Yes** — `Figure.source.line_item` requires the company's own wording |
