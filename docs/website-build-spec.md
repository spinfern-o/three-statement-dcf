...
**Status: authoritative brief. Verbatim as supplied 2026-09-16; not edited by
implementation work.**

This is the specification for expanding the repository from the working Python
calculation engine into a full web application. It was supplied by the repository
owner and appended to `README.md`; it was moved here unchanged so that `README.md`
can describe what the project *is*, while this file records what it is *to become*.

Implementation status against this document is tracked in
[`decision-ledger.md`](decision-ledger.md). Section 2 of the specification below
requires that every listed decision be recorded as Confirmed or OPEN before the
work it blocks begins.

Two notes recorded during the first read, neither of which changes the text below:

- **Section 0** refers to an attached file `Homework 2 (1)(1).png`. No such file
  reached the implementing session — it is absent from the repository and from the
  environment — so nothing was excluded on its account and no financial value has
  ever been taken from it. If it was meant to be supplied, it still needs to be.
- **Section 26** records design observations of findash.ai. That site renders its
  content with JavaScript, so those specific colors and typefaces could not be
  independently verified from the page source. They are treated below as the
  owner's stated observations, and Section 6.4's requirement that token values pass
  contrast testing governs the values actually adopted.
