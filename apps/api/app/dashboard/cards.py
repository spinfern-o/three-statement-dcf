...
STATUS_LEGEND = (
    ...
    (ModelStatus.FORECAST_READY,
     "Every required assumption is an answer (14.1) and the forecast builds for "
     "every period."),
...
def portfolio_cards(models: tuple[tuple[object, Standing], ...]) -> tuple[Card, ...]:
    ...
    total = len(models)
    ready = sum(
        1 for _, standing in models
        if standing.status.rank >= ModelStatus.VALUATION_READY.rank
    )
    needs_review = sum(
        1 for _, standing in models if standing.status is ModelStatus.NEEDS_REVIEW
    )
    unresolved = sum(standing.unresolved_count for _, standing in models)

    return (
        Card(
            label="Models", value=str(total),
            period="all", scenario="",
            source="counted from the documents in this store",
            status="live", tone="neutral",
        ),
        Card(
            label="Valuation ready", value=str(ready),
            period="current state", scenario="base",
            source="computed from each model's own contents",
            status="computed", tone="success" if ready else "neutral",
        ),
        Card(
            label="Needs review", value=str(needs_review),
            period="current state", scenario="",
            source="every fact decided and the metadata confirmed",
            status="computed",
            tone="warning" if needs_review else "success",
        ),
        Card(
            label="Unresolved items", value=str(unresolved),
            period="current state", scenario="",
