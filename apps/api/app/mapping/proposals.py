...
BLOCKED = (
    ...
    (
        re.compile(
            r"(?i)\bper\s+share\b|\bearnings\s+per\s+share\b|\bweighted[- ]average\s+(number\s+of\s+)?shares\b"
        ),
...
RULES = (
    ...
    _rule(
        ...
        EXACT,
        "the whole label is a revenue line",
    ),
    _rule(
        accounts.REVENUE,
        r"revenue\s+from\s+contracts\s+with\s+customers",
        EXACT,
        "matched the IFRS 15 / ASC 606 revenue caption",
    ),
    _rule(
        accounts.REVENUE,
        r"^(total\s+)?(net\s+)?(sales|revenues?)\b",
        STRONG,
        "the label begins with a revenue term",
    ),
    _rule(accounts.REVENUE, r"\bturnover\b", STRONG, "matched 'turnover', the UK term for revenue"),
    _rule(
        accounts.COGS,
        r"^cost\s+of\s+(goods\s+sold|sales|revenue|services)",
        EXACT,
        "matched a cost-of-sales caption",
    ),
    _rule(
        accounts.COGS,
        r"\bcost\s+of\s+(goods|sales|revenue)\b",
        STRONG,
        "matched 'cost of sales' in the label",
    ),
    _rule(
        accounts.GROSS_PROFIT,
        r"^gross\s+(profit|margin|income)$",
        EXACT,
        "the whole label is a gross profit line",
    ),
    _rule(
        accounts.OPERATING_EXPENSES,
        r"^(selling,?\s+general\s+and\s+administrative|sg&a)",
        EXACT,
        "matched the SG&A caption",
    ),
    _rule(
        accounts.OPERATING_EXPENSES,
        r"^research\s+and\s+development",
        EXACT,
        "matched the R&D caption",
    ),
    _rule(
        accounts.OPERATING_EXPENSES,
        r"^(total\s+)?operating\s+(expenses|costs)",
        EXACT,
        "matched a total operating expense caption",
    ),
    _rule(
        accounts.OPERATING_EXPENSES,
        r"\b(restructuring|impairment|amortisation\s+of\s+acquired)",
        STRONG,
        "an operating charge presented within operating income",
    ),
    _rule(
        accounts.OPERATING_EXPENSES,
        r"\b(marketing|distribution|administrative)\s+(expense|cost)",
        STRONG,
        "an operating expense category",
    ),
    _rule(
        accounts.EBIT,
        r"^(operating\s+(income|profit)|income\s+from\s+operations)$",
        EXACT,
        "the whole label is an operating income line",
    ),
    _rule(
        accounts.INTEREST_EXPENSE,
        r"^interest\s+expense",
        EXACT,
        "matched an interest expense caption",
    ),
    _rule(
        accounts.INTEREST_EXPENSE,
        r"^(finance|financing)\s+costs?$",
        STRONG,
        "matched the IFRS finance-cost caption",
    ),
    _rule(
        accounts.OTHER_INCOME_EXPENSE,
        r"^(other|non-?operating)\s+(income|expense)",
        EXACT,
        "matched an other-income caption",
    ),
    _rule(
        accounts.OTHER_INCOME_EXPENSE,
        r"^interest\s+income",
        STRONG,
        "interest income is non-operating in this chart",
    ),
    _rule(
        accounts.OTHER_INCOME_EXPENSE,
        r"\b(foreign\s+(exchange|currency)|equity\s+in\s+earnings|gain\s+on\s+(sale|disposal))",
        STRONG,
        "a non-operating gain or loss",
    ),
    _rule(
        accounts.PRETAX_INCOME,
        r"income\s+before\s+(income\s+)?tax",
        EXACT,
        "matched a pre-tax income caption",
    ),
    _rule(
        accounts.PRETAX_INCOME,
        r"^(profit|earnings)\s+before\s+tax",
        EXACT,
        "matched a pre-tax income caption",
    ),
    _rule(
        accounts.TAXES,
        r"^(income|corporate)\s+tax(es|ation)?\s+(expense|provision|charge)",
        EXACT,
        "matched an income tax expense caption",
    ),
    _rule(
        accounts.TAXES,
        r"^provision\s+for\s+income\s+tax",
        EXACT,
        "matched the US provision-for-income-taxes caption",
    ),
    _rule(
        accounts.TAXES,
        r"^(income\s+)?tax(es)?$",
        WEAK,
        "the label is just 'tax' -- confirm it is INCOME tax and not payroll, sales or property tax, which are operating cost",
    ),
    _rule(
        accounts.NET_INCOME,
        r"^net\s+(income|earnings|profit)",
        EXACT,
        "matched a net income caption",
    ),
    _rule(
        accounts.NET_INCOME,
        r"^(profit|loss)\s+for\s+the\s+(year|period)",
        EXACT,
        "matched the IFRS profit-for-the-period caption",
    ),
    # --- balance sheet ----------------------------------------------------
    _rule(
        accounts.CASH,
        ...
        EXACT,
        "matched the cash and equivalents caption",
    ),
    _rule(accounts.CASH, r"^cash$", STRONG, "the whole label is 'cash'"),
    _rule(
        accounts.ACCOUNTS_RECEIVABLE,
        r"^(accounts|trade)\s+receivable",
        EXACT,
        "matched a trade receivables caption",
    ),
    _rule(
        accounts.ACCOUNTS_RECEIVABLE,
        r"^receivables",
        STRONG,
        "matched 'receivables' -- confirm these are TRADE receivables",
    ),
    ...
    _rule(
        ...
        r"^stock$",
        WEAK,
        "'stock' is inventory in UK usage and share capital in US usage -- confirm which",
    ),
    _rule(
        accounts.OTHER_CURRENT_ASSETS,
        r"^other\s+current\s+assets",
        EXACT,
        "the whole label names the other-current-asset line",
    ),
    _rule(
        accounts.OTHER_CURRENT_ASSETS,
        r"^(other|prepaid|prepayments)",
        STRONG,
        "matched an other-current-asset caption",
    ),
    _rule(
        accounts.OTHER_CURRENT_ASSETS,
        r"\b(short-?term\s+investments|restricted\s+cash|contract\s+assets)",
        STRONG,
        "an item this chart routes to other current assets",
    ),
    _rule(
        accounts.PPE_NET,
        r"^property,?\s+(plant\s+and\s+)?equipment",
        EXACT,
        "matched a property, plant and equipment caption",
    ),
    _rule(
        accounts.PPE_NET,
        r"^(fixed|tangible)\s+assets",
        STRONG,
        "matched the UK/IFRS tangible assets caption",
    ),
    ...
    _rule(
        ...
        r"^other\s+(non-?current|long-?term)\s+assets",
        EXACT,
        "the label says NON-current, which the generic 'other' rule would miss",
    ),
    _rule(
        accounts.OTHER_NONCURRENT_ASSETS,
        r"^(goodwill|intangible)",
        STRONG,
        "the chart has no goodwill or intangibles line (12.2.e); it routes here, and that is worth recording on the mapping",
    ),
    _rule(
        accounts.OTHER_NONCURRENT_ASSETS,
        r"\b(right-?of-?use|deferred\s+tax\s+assets?|equity[- ]method\s+investments?)",
        STRONG,
        "a non-current asset this chart routes to the residual line",
    ),
    _rule(accounts.TOTAL_ASSETS, r"^total\s+assets$", EXACT, "the whole label is total assets"),
    _rule(
        accounts.ACCOUNTS_PAYABLE,
        r"^(accounts|trade)\s+payable",
        EXACT,
        "matched a trade payables caption",
    ),
    _rule(
        accounts.OTHER_CURRENT_LIABILITIES,
        r"^other\s+current\s+liabilit",
        EXACT,
        "the whole label names the other-current-liability line",
    ),
    _rule(
        accounts.OTHER_CURRENT_LIABILITIES,
        r"^(accrued|deferred\s+revenue|contract\s+liabilit)",
        STRONG,
        "an item this chart routes to other current liabilities",
    ),
    _rule(
        accounts.DEBT,
        r"^(long-?term\s+)?(debt|borrowings|loans)",
        EXACT,
        "matched a borrowings caption",
    ),
    _rule(
        accounts.DEBT,
        r"\b(notes\s+payable|current\s+portion\s+of\s+long-?term\s+debt|finance\s+lease\s+liabilit|lease\s+liabilit)",
        STRONG,
        "interest-bearing, so it belongs in debt (decision 2.4.j for leases)",
    ),
    _rule(
        accounts.OTHER_NONCURRENT_LIABILITIES,
        r"^other\s+(non-?current|long-?term)\s+liabilit",
        EXACT,
        "the label says NON-current; the same trap as other non-current assets",
    ),
    _rule(
        accounts.OTHER_NONCURRENT_LIABILITIES,
        r"\b(deferred\s+tax\s+liabilit|pension|post-?retirement|provisions)",
        STRONG,
        "a non-current liability this chart routes to the residual line",
    ),
    _rule(
        accounts.TOTAL_LIABILITIES,
        r"^total\s+liabilities$",
        EXACT,
        "the whole label is total liabilities",
    ),
    _rule(
        accounts.COMMON_EQUITY,
        r"^(common|ordinary)\s+stock|^share\s+capital|^additional\s+paid-?in\s+capital|^treasury\s+stock|accumulated\s+other\s+comprehensive",
        EXACT,
        "a contributed-capital or reserve line",
    ),
    _rule(
        accounts.RETAINED_EARNINGS,
        r"^(retained\s+earnings|accumulated\s+(deficit|profits))",
        EXACT,
        "matched a retained earnings caption",
    ),
    _rule(
        accounts.TOTAL_EQUITY,
        r"^total\s+(stockholders'?|shareholders'?|owners'?)?\s*equity$",
        EXACT,
        "the whole label is total equity",
    ),
    # --- cash flow statement ----------------------------------------------
    _rule(
        ...
        r"^depreciation",
        EXACT,
        "matched a depreciation caption",
    ),
    _rule(
        accounts.DEPRECIATION_AMORTIZATION,
        r"^amorti[sz]ation",
        STRONG,
        "matched an amortisation caption",
    ),
    _rule(
        accounts.STOCK_BASED_COMP,
        r"(stock|share)-?based\s+(compensation|payment)",
        EXACT,
        "matched a share-based payment caption",
    ),
    _rule(
        accounts.CHANGE_IN_NWC,
        r"changes?\s+in\s+(operating\s+)?(working\s+capital|assets\s+and\s+liabilities)",
        EXACT,
        "matched a working-capital movement caption",
    ),
    _rule(
        accounts.CFO,
        r"^(net\s+)?cash\s+(provided\s+by|from|used\s+in)\s+operating",
        EXACT,
        "matched the operating cash flow subtotal",
    ),
    _rule(
        accounts.CAPEX,
        r"(purchases?|acquisitions?|additions?)\s+of\s+(property|plant|equipment|fixed\s+assets)",
        EXACT,
        "matched a capital expenditure caption",
    ),
    _rule(
        accounts.CAPEX, r"^capital\s+expenditures?", EXACT, "the whole label is capital expenditure"
    ),
    _rule(
        accounts.ACQUISITIONS,
        r"acquisitions?\s+(of\s+)?(business|subsidiar)|business\s+combinations?",
        EXACT,
        "matched a business-combination caption",
    ),
    _rule(
        accounts.CFI,
        r"^(net\s+)?cash\s+(provided\s+by|from|used\s+in)\s+investing",
        EXACT,
        "matched the investing cash flow subtotal",
    ),
    _rule(
        accounts.DEBT_ISSUANCE,
        r"^proceeds\s+from\s+(the\s+)?(issuance\s+of\s+)?(debt|borrowings|notes|long-?term)",
        EXACT,
        "matched a borrowing-proceeds caption",
    ),
    _rule(
        accounts.DEBT_REPAYMENT,
        r"(repayments?|payments?)\s+(of|on)\s+(debt|borrowings|notes|long-?term)",
        EXACT,
        "matched a debt-repayment caption",
    ),
    _rule(
        accounts.SHARE_REPURCHASES,
        r"(repurchases?|buy-?backs?)\s+of\s+(common\s+)?(stock|shares)",
        EXACT,
        "matched a share-repurchase caption",
    ),
    _rule(
        accounts.DIVIDENDS, r"^dividends?\s+(paid|to)", EXACT, "matched a dividends-paid caption"
    ),
    _rule(
        accounts.DIVIDENDS,
        r"^dividends?$",
        STRONG,
        "the whole label is 'dividends' -- confirm it is dividends PAID, not declared",
    ),
    _rule(
        accounts.CFF,
        r"^(net\s+)?cash\s+(provided\s+by|from|used\s+in)\s+financing",
        EXACT,
        "matched the financing cash flow subtotal",
    ),
)
