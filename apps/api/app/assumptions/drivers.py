...
DRIVERS = (
    ...
    Driver(
        "opex_amount",
        "Operating expenses, stated absolutely",
        "currency",
        "STEP 14",
        alternative="opex_pct_revenue",
    ),
    Driver(
    ...
    Driver(
        "other_current_assets_pct_revenue", "Other current assets on revenue", "ratio", "STEP 17"
    ),
    Driver(
        "other_current_liabilities_pct_revenue",
        "Other current liabilities on revenue",
        "ratio",
        "STEP 17",
    ),
    Driver(
    ...
    Driver("sbc_pct_revenue", "Stock compensation on revenue", "ratio", "STEP 20", required=False),
    Driver("share_repurchases", "Share repurchases", "currency", "STEP 21", required=False),
    Driver(
        "dividends_amount",
        "Dividends, stated absolutely",
        "currency",
        "STEP 21",
        alternative="dividend_payout_ratio",
        required=False,
    ),
    Driver(
        ...
        "ratio",
        "STEP 21",
        alternative="dividends_amount",
        required=False,
    ),
    Driver("acquisitions", "Acquisitions", "currency", "STEP 18", required=False),
    Driver(
        "other_income_expense", "Other income and expense", "currency", "STEP 15", required=False
    ),
    Driver("other_operating", "Other operating cash flows", "currency", "STEP 22", required=False),
    Driver("other_investing", "Other investing cash flows", "currency", "STEP 22", required=False),
    Driver("other_financing", "Other financing cash flows", "currency", "STEP 22", required=False),
)
...
def driver(code: str) -> Driver:
    ...
    if base not in BY_CODE:
        raise KeyError(f"{code!r} is not a forecast driver. Known: {', '.join(sorted(BY_CODE))}")
    return BY_CODE[base]
